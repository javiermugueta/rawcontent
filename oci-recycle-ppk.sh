...
newfp="$(oci iam user api-key upload --user-id $ociuser --key-file $pubfile | jq -r '.data.fingerprint')"
#
...
if  [[ $newfp == "" ]]; then
    ...
    fps=$(oci iam user api-key list --user-id $ociuser | jq '.data[]' | jq -r ."fingerprint")
    for fp in $fps 
    do
        if [[ $fp != $newfp ]]; then
